...
class WalkTimeView: UIView {
    ...
    public func set(distance: CLLocationDistance, timeToWalk: TimeInterval) {
        ...
        if let timeString = formatters.positionalTimeFormatter.string(from: timeToWalk) {
            let fmt = OBALoc("walk_time_view.distance_time_fmt", value: "%@, %@: arriving at %@", comment: "Format string with placeholders for distance from stop, walking time to stop, and predicted arrival time. e.g. 1.2 km, 17m: arriving at 09:41 A.M.")
        ...
        if let timeString = formatters.accessibilityPositionalTimeFormatter.string(from: timeToWalk) {
            let fmt = OBALoc("walk_time_view.accessibility_value", value: "%@. Takes %@ to walk, arriving at %@", comment: "Accessibility string with placeholders for distance from stop, walking time to stop, and predicted arrival time. e.g. 1.2 km. Takes 17 minutes to walk, arriving at 9:41 am")
